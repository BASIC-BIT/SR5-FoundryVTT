...
/**
 * The most basic chummer actor data importer, meant to handle one or more Chummer5a data <type>.xml file.
 * 
 * Generic type ActorDataType is the items data type DataImporter creates per entry in that Chummer5a data .xml file.
 */
export abstract class DataImporter<ActorDataType, ActorSystemDataType> {
    ...
    /**
     * Get complete actor data.
     * 
     * NOTE: We use temporary actors to have a full set of actor data instead of just
     *       system model data that game.model.Actor would give us.
     */
    public GetDefaultData({type}:{type:any}) {
        return DataDefaults.baseItemData<ActorDataType, ActorSystemDataType>({type});
    }
    ...
    /**
     * Parse the specified jsonObject and return Actor representations.
     * @param chummerData The JSON data to parse.
     * @returns An array of created objects.
     */
    public abstract Parse(chummerData: object): Promise<Actor>;
